...
const Modal = ({
    ...
    <span className="close cursor-pointer text-xl" onClick={closeModal}>
      &times;
                ...
                <p className="modal-combat-header text-center text-lg font-bold mb-4">
                ...
                <div className="button-wrapper flex justify-around items-center mb-4">
                    ...
                    <button
                        ...
                        className="defend-modal-btn bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700 transition-colors"
                        disabled={!canAct}
                    >
                        🛡️Parry
                    </button>
                    <button
                        className="close-modal-btn bg-gray-500 text-white py-2 px-4 rounded hover:bg-gray-700 transition-colors"
                        onClick={closeModal}
                    >
                        🏃Run
                    </button>
                </div>
                <div className="health-status mb-4">
                    <div className="status-wrapper flex justify-between">
                        <div className="player-status text-center">
                            <h3 className="text-lg font-bold">👽Player</h3>
                            <div className="player-img-wrapper">
                                <div className={`player-img ${playerDamaged ? "shake flashRed" : ""}`}></div>
                                <div>{playerDamaged && (hit.play(), "")}</div>
                            </div>
                            <p>{getHealthEmoji(playerHealth)} Health: {playerHealth}</p>
                            <p>💪Strength: {playerStrength}</p>
                        </div>
                        <div className="enemy-status text-center">
                            <h3 className="text-lg font-bold">{isBoss ? "💀Boss" : "🤖Enemy"}</h3>
                            <div className="enemy-img-wrapper">
                                <div
                                    className={`${isBoss ? "boss-img" : "enemy-img"} ${enemyDamaged ? "shake" : ""}`}></div>
                            </div>
                            <p>{getHealthEmoji(enemyHealth)} Health: {enemyHealth}</p>
                ...
                <div className="timer text-center">
                    <p>
                        {isTimerActive ? `⏲️Time Remaining: ${timer} seconds` : "Wait for timer to start..."}
                    </p>
                </div>
            </div>
        </div>
